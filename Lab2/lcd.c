...
void lcd_rotatingBanner(char dispText[])
{
    ...
    //Fills str with spaces and sets null char
    for (i = 0; i < 20; i++)
    {
        str[i] = ' ';
    }
    ...
    //Loops given message infinitely
    while (1)
    {
        //puts message on screen one character at a time, every 0.3 seconds, from the right side
        for (i = 0; i < len; i++)
        {
            lcd_printf("%s", str);
            timer_waitMillis(300);

            //moves characters to the right in str and adds new characters from dispText[]
            for (j = 0; j < 20; j++)
            {
                if (str[j + 1] == '\0')
                {
        ...
        //shifts the message off screen and fills the end with spaces
        for (i = 0; i < 20; i++)
        {
            lcd_printf("%s", str);
            timer_waitMillis(300);

            //moves characters to right in str and adds spaces behind
            for (j = 0; j < 20; j++)
            {
                if (str[j + 1] == '\0')
                {
